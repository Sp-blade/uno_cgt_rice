<?php
	...
	$productBaseUnit = strtolower(trim((string) ($_GET['product_base_unit'] ?? 'pc'))) === 'kg' ? 'kg' : 'pc';
	...
	switch($mainMenu){
		...
		case "view_sale":
			if (isset($_GET['expense_category']) || isset($_GET['expense_amount'])) {
				...
				$deliveryNo = mysqli_real_escape_string($connectDB, $_GET['deliveryNo']);
				$saleDate = mysqli_real_escape_string($connectDB, $_GET['sale_date']);
		...
		case "product_list":
			...
			$sql_addProduct = "INSERT INTO products (ProductName, ProductType, ProductBaseUnit, ProductPrice, SellingPrice, StockLimit, CanConvertToKg, KgEquivalentQty, ParentProduct_ID, IsSubProduct, IsActive) VALUES ('$productName','$productType','$productBaseUnit','$productPrice','$sellingPrice','$stockLimit','$canConvertToKg','$kgEquivalentQty','$parentProductId','$isSubProduct','1')";
