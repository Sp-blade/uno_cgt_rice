	...
	if ($lpgResult && $lpgResult->num_rows > 0) {
		while ($row = $lpgResult->fetch_assoc()) {
			...
			$lpgRows[] = [
				...
				'filled_stock' => round((float) ($row['FilledStock'] ?? 0), 2),
					...
					<?php if (!empty($lpgRows)): ?>
						<?php foreach ($lpgRows as $row): ?>
							<tr>
								...
								<td class="text-end"><?php echo number_format($row['filled_stock'], 2); ?></td>
								<td class="text-end"><?php echo number_format($row['empty_new'], 2); ?></td>
								<td class="text-end"><?php echo number_format($row['empty_old'], 2); ?></td>
							</tr>
						<?php endforeach; ?>
					<?php else: ?>
						<tr><td colspan="4" class="empty-state">No LPG products found. Set Product Type to LPG on the Product List.</td></tr>
					<?php endif; ?>
				</tbody>
			</table>
		</div>
